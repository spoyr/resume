...
{
    if (\KS\Controllers\ExchangeQueue::isExchangeAllowed('catalog', 'import'))
    {
...
elseif (($_GET["mode"] == "import") && $ABS_FILE_NAME)
{
	...
	{
        /**
         * вызываем функцию обработки xml файлов. В случае, если в возвращенном массиве
         * есть ошибки, отправляем статус failure и текст ошибки, обмен прерывается.
         */
        $errors = SyncWithCatalogs();
        if (empty($errors))
        {
            $NS["STEP"] = 10;
        }
        ...
        {
            logFile($errors, false, 'errors.txt');
            foreach ($errors as $id => $error)
            {
                $strError .= $error.PHP_EOL;
            }
            $strError .= GetMessage("CC_BCI1_CUSTOM_EXCHANGE_ERROR");
        }
	}
	elseif ($NS["STEP"] == 3)
	{
...
elseif ($_GET["mode"]=="end_exchange")
{
    if (\KS\Controllers\ExchangeQueue::isExchangeAllowed('catalog', 'import'))
    {
        \KS\Controllers\ExchangeQueue::setCompleteExchange('catalog', 'import');
    }
}
else
{
